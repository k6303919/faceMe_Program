...
def search_similar_face(command_args,image):
    """
    print(command_args)
    if command_args.image_path == "":
        return show_help_and_exit(command_args.command, "-photo")
    ...
    global faceMe_sdk
    ret, image = faceMe_sdk.convert_to_faceMe_image(command_args.image_path)
    #image =  cv2.imread(command_args.image_path)
    print(type(image))
    """
    ret, image = faceMe_sdk.convert_opencvMat__to_faceMe_image(command_args.image_path)
    if FR_FAILED(ret):
        print("Convert to FaceMe image failed")
        return
...
def recognize_face(command_args, image):
    if command_args.image_path == "":
        return show_help_and_exit(command_args.command, "-photo")
    global faceMe_sdk
    ret, image = faceMe_sdk.convert_opencvMat__to_faceMe_image(image)
    #ret, image = faceMe_sdk.convert_to_faceMe_image(command_args.image_path)
    if FR_FAILED(ret):
        print("Convert to FaceMe image failed")
        return
...
if __name__ == "__main__":
    #main(['searchsimilarface', '-photo', '0820_KEVIN.jpg'])
    main(sys.argv[1:])
